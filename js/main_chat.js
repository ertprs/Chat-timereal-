...
$(function(){
 ...
 $('#maisbotoes').click(function(){ 
     $('#divBtnAcessarTransferencia').toggle()
     });
 
 
    if(screen.width < 880) {
       $('#divBtnAcessarTransferencia').hide();
         $('#frame .content').hide();
    }
    // MOBILE transition

    $('#voltarmobile').click(function(){
        
      $('#sidepanel').show(1000);
       $('#frame .content').hide();
      
      //$('#frame .content').hide(1000);
  
    });
    setInterval(function(){
        
        // dark and light theme TEST BETA
        $('#trocartheme img').click(function(){$(this).addClass("darktheme"); $('body').prepend('<div class="newstyle"><style type="text/css">#txtNewMessage, .emojionearea.input-msg, .conversation-compose, #frame #sidepanel #contacts ul li.contact.active, #frame, .message:after, body, #profile, #sidepanel, .contact-profile, .conversation-container{    border-color: #8080804a !important;background: white !important} *, .message {color: gray !important} .contact .btn.btn-primary.btn-clean {color: white !important} .message.received:after {border-color: #ebebeb !important}.message.sent {background-color: #0a80ff !important; color: #fff !important;} .message.received {    background: #ebebeb  !important; padding: 1em !important}</style></div>');  $('.darktheme').click(function(){$('.newstyle').remove(); $(this).removeClass('darktheme'); }); }); 
    }, 1000);
    ...
    // SHIFT MATEUS ANJOS FOR WHATSCOMPANY
$("#txtNewMessage").keydown(function(e){
	if(e.shiftKey) {
	$(this).val($(this).val() + "\n");
	}
});

// sem tempo de espera quando envia mensagem
$('.conversation-compose .send .circle').click(function(){
	var Msg123 = $('#txtNewMessage').val(); 
	var idultimamensagem = $('.message.sent').last().attr('id');
	var dataNow = new Date();
   var localdate =  dataNow.getHours() + ':' + dataNow.getMinutes(); 
   if(dataNow.getMinutes < 10) {  
   localdate =  dataNow.getHours() + ':0' + dataNow.getMinutes() ; 
   } 
   
	var htmlmsg = '<span class="metadata"><span class="time2">'+localdate+'</span><span class=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="15" id="msg-dblcheck" x="2047" y="2061"><path d="M15.01 3.316l-.478-.372a.365.365 0 0 0-.51.063L8.666 9.88a.32.32 0 0 1-.484.032l-.358-.325a.32.32 0 0 0-.484.032l-.378.48a.418.418 0 0 0 .036.54l1.32 1.267a.32.32 0 0 0 .484-.034l6.272-8.048a.366.366 0 0 0-.064-.512zm-4.1 0l-.478-.372a.365.365 0 0 0-.51.063L4.566 9.88a.32.32 0 0 1-.484.032L1.892 7.77a.366.366 0 0 0-.516.005l-.423.433a.364.364 0 0 0 .006.514l3.255 3.185a.32.32 0 0 0 .484-.033l6.272-8.048a.365.365 0 0 0-.063-.51z" fill="#92a58c"></path></svg></span></span>';
	var z = document.createElement('div'); // is a node


z.innerHTML = "<div class='message sent' id="+idultimamensagem+">"+Msg123+" "+htmlmsg+"</div>";
	var el = document.getElementsByClassName("conversation-container")[0];
	el.appendChild(z);  
}); 
...
/* function  Carregamensagensjson() {
     	$(conversation).html("");
                           var y = $('.contact.active').find('.name').text();
                           //localStorage.getItem(y);
                           	var z = document.createElement('div'); // is a node


                            z.innerHTML = localStorage.getItem(y);
                        	var el = document.getElementsByClassName("conversation-container")[0];
...
function StartApp() {
 ...
 if(localStorage.getItem("htmlcontact") != null) {  
                      var thtmlnew =   localStorage.getItem("htmlcontact");
                      //console.log(thtmlnew);
                      
                      // adicionar contato ao dom
                      $("#myContacts").html(thtmlnew);
                      
                      // função click nos contatos
...
$(document).ready(function () {
  ...
  $(conversation).html('');
    ...
    function mmsgcontact(){
    function myStopFunction() {
  clearInterval(myMsg);
}

   var myMsg =  setInterval(function(){
         if($('.msglast').length){
              $(conversation).html("");
             myStopFunction();
             //console.log("Função interrompida");
         } else {
                $('#myContacts li').each(function(){
	...
	$('.message').each(function() {
		...
		console.log(nome);
		var msg = $(this).text();
		var lista_msg = JSON.parse(localStorage.getItem(nome) || '[]');
		lista_msg.push({
			msg: msg
		});
		localStorage.setItem(nome, JSON.stringify(lista_msg));
	});
	//console.log('Conversa salva.');
}
salvarMsg();        var tagselect = $('#tag option:selected').text();
            var metadata  =  $('.message').last().find(".metadata").text(); 
            //console.log(metadata);
            $('.message').find(".metadata").remove(); 
            var mmsglast = $('.message').last().text();
            $(conversation).html('');
            $(conversation).show();
          if(mmsglast.length <= 0) {
               $(this).find('.preview').append("<div class='msglast'>\n Arquivo recebido <div class='msgdata' style='float: right'>" + metadata + "</div></div>");
          } else { 
            $(this).find('.preview').append("<div class='msglast'>\n" + mmsglast + "<div class='msgdata' style='float: right'>" + metadata + "</div></div>");
          }
           //$(this).find('.meta').before("<div class='btn btn-primary btn-clean'>"+tagselect+"</div>");
...
setTimeout(function(){
      $('#myContacts li').each(function(){
var htmlli = $(this).parents("ul").html();
 localStorage.setItem("htmlcontact", htmlli);
      ...
      $('#frame .content').show();
            ...
            if (botao == 0 || botao == 1) {
                if (telefone != $('#txtTelefoneAtivo').val()) //Se o Contato escolhido for diferente do ativo
                {
                    ...
                    //var contato_ativo = texto.replace(telefone, "");
var contato_ativo = $(this).find('.name').text();
                    $('#contact-profile span').text(contato_ativo.trim());
...
// teste habilitar abaixo - mateus;
                    Carrega_Mensagens();
                    //Carregamensagensjson();
                ...
                if (response) {
                    ...
                    for (var i = 0; response.length > i; i++) {
                        ...
                        if (response[i].entrada_saida === "1") {
                            ...
                            // *************************
if($('img.emojioneemoji').is(":visible")) {

    $('#txtNewMessage').val("Emoji");
        
    }
                            // APRESENTA A MENSAGEM NA TELA
                             $('.message.sent').last().remove();
...
function Pesquisa_Contatos(nome) {
    ...
    $.ajax({
        ...
        data: {contato: nome},
        dataType: 'json',
        async: true,                   // O Navegador, primeiramente, completa a requisição   29/01/2019
        success: function (response) {
            ...
            if (response) {
                ...
                for (var i = 0; response.length > i; i++) {
                    ...
                    if (response[i].status == '1') {
						...
						<div class="wrap">\
							<img src="images/contact.png" alt="" style="margin-top: 5px; height: 45; width: 45px;" />\
							<div class="meta">\
								<p class="name">' + utf8_decode(response[i].nome) + '</p>\
								<p class="preview">' + response[i].numero + '</p>\
								<p style="background: #00BFFF; text-align: center; max-width: 150px; float: right; right: 0px; bottom: 20px; position: absolute; border-radius: 2px 2px 2px 2px; padding: 3px; color: #fff; font-size: 11px; font-family: arial;">' + response[i].tag + '</p>\
							</div>\
						</div>\
					   </li>');
                    } else {
    ...
    if (telefone.length < 10) {
        ...
        if ($.trim(nome) != '' && $.trim(telefone) != '' && $.trim(mensagem) != '') {
            ...
            $.ajax({
                ...
                dataType: 'json',
                async: true,                     // PARA AGUARDAR O PROCESSAMENTO
                success: function (response) {
...
// desnecessário, remoção por MateusAnjos.
                //setTimeout(function () {
...
function maisde25(){
    if ($('#bulk .message').length > 25) {
	var tcont = $('#bulk .message').length;
	var cont = tcont - 25;
	var cont2 = cont;
	while (cont != 0) {
		$('.message:nth-child(' + cont + ')').hide();
		cont = cont - 1;
	}
	var esperamsg = 100;
	var timeout = null;

	$('.conversation-container').bind('scroll', function() {
		clearTimeout(timeout);
		timeout = setTimeout(function() {

			if ($('.conversation-container').scrollTop() == 0) {
				// caso tenha mais de 25
				if (cont2 > 25) {
					var cont3 = cont2 - 25;
					//console.log(cont3);
					var contfixo = 0;
					...
					while (cont3 != cont2) {
						...
						$('.message:nth-child(' + cont2 + ')').show();
						cont2 = cont2 - 1;
					}
				}
				// certinho caso tenha menos de 25
				else {
					while (cont2 != 0) {
						//console.log($('.message:nth-child('+cont+')'))
						$('.message:nth-child(' + cont2 + ')').show();
						cont2 = cont2 - 1;
					}
				}

							   ...
							   else 
							   {
								   ...
								   $result = mysqli_query($con,$sql );
									...
									// ***********************************
										...
										// JULIO 21/05/2020 ÀS 20:09
										$sql = "SELECT name FROM tags LIMIT 1";
										$result = mysqli_query($con,$sql );

										if(mysqli_num_rows($result)) {
											$row=mysqli_fetch_assoc($result);
											$name = $row["name"];          // verifica a 1ª tag
											
										}	
										// JULIO 21/05/2020 ÀS 20:09 até aqui
										
										$data_rg = date("Y-m-d");
										$sql = "INSERT INTO contatos (nome, numero, status,posicao, tag, user_cli, data_rg) "; 
										$sql .= "VALUES ('$nome_cliente', '$whatsapp', '0','$posicao', '$name', '$user_cli', '$data_rg')";
